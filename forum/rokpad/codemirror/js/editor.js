...
var Editor = (function(){
  ...
  function SearchCursor(editor, string, fromCursor, caseSensitive) {
    this.editor = editor;
    ...
    this.matches = (target.length == 1) ?
      ...
      function() {
		var text = cleanText(self.history.textAfter(self.line).slice(self.offset));
		if (caseSensitive) var match = text.toLowerCase().indexOf(string.toLowerCase());
        else var match = text.indexOf(string);
        if (match > -1)
      ...
      function() {
		...
		if (caseSensitive) var match = firstLine.toLowerCase().lastIndexOf(target[0].toLowerCase());
        else var match = firstLine.lastIndexOf(target[0]);
  ...
  Editor.prototype = {
    ...
    getSearchCursor: function(string, fromCursor, caseSensitive) {
      return new SearchCursor(this, string, fromCursor, caseSensitive);
    },
    ...
    keyDown: function(event) {
      ...
      if (code == 13) { // enter
      ...
      else if (code == 35 && !event.shiftKey) { // end
		...
		else if ((event.shiftKey && code == 73) || code == 73) { // I
		  this.rokdev.indent();
		  event.stop();
		}
		else if (code == 76) { // L
		  this.rokdev.gotoLine(true);
		  event.stop();
		}
      }
	  else if (event.altKey && code == 70) { // F
		this.rokdev.search(true);
		event.stop();
	  }
    },
    ...
    handleTab: function(direction) {
      if (this.options.tabMode == "spaces")
        select.insertTabAtCursor(this.win, indentUnit);
      else
